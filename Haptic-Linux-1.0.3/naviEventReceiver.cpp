		...
		case EMIE_MOUSE_WHEEL:
			if(!clickDown){
				...
				// scroll out
					...
					if(user->isInside)
						user->setPosition(vector3df(87,10,127));
					user->setVisible(true);
				}
			}
			break;
	 ...
	 if (event.EventType == EET_GUI_EVENT)
	{
		...
		if(event.GUIEvent.EventType == EGET_BUTTON_CLICKED)
		{
			if(event.GUIEvent.Caller->getID() == BUTTON_YES){
				//finalId = clickedMarkerIndex;
				finalId = map->getMarkerButtons()[clickedMarkerIndex].id;
				printf("[DEBUG](%s-%d)clickedMarkerIndex:%d\n",__func__,__LINE__,clickedMarkerIndex);
				popsy.closePopUp();
				roadIsCalculated = false;
			}
...
int naviEventReceiver::getClickedMarkerId(){
    printf("[DEBUG](%s-%d)finalID:%d\n",__func__,__LINE__,finalId);
	return finalId;
}
